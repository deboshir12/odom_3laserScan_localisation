...
class RangeOdomCorrectorMap(Node):
    def __init__(self):
        super().__init__('range_odom_corrector_map')
        ...
        # Частая публикация TF
        self.declare_parameter('tf_publish_period', 0.01)

        # Редкая коррекция по дальномерам
        ...
        # Ограничение скачка за одну коррекцию
        self.declare_parameter('max_correction_xy', 0.08)      # 3 см
        self.declare_parameter('max_correction_yaw', 0.08)     # ~2.8 градуса
        ...
        # Текущее публикуемое преобразование
        self.map_to_odom = (0.0, 0.0, 0.0)

        # Последняя вычисленная "сырая" коррекция
        self.target_map_to_odom = (0.0, 0.0, 0.0)
        ...
        self.get_logger().info('Range odom corrector with occupancy map started')
    ...
    def apply_limited_smoothed_update(self, new_map_to_odom):
        ...
        # Ограничение максимального скачка
    ...
    def publish_map_to_odom(self, tf_pose):
        if self.odom_msg is None:
            return
